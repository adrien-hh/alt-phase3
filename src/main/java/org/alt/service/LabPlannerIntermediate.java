...
public class LabPlannerIntermediate {
  ...
  public LabOutputIntermediate planifyLab(LabInputIntermediate input) {
    ...
    MetricsIntermediate metrics =
        metricsCalculator.computeMetrics(input, builtSchedule.entries(), builtSchedule.conflicts());
  ...
  private BuiltSchedule buildSchedule(
    ...
    Map<String, LocalTime> technicianFreeAt =
        technicians.stream()
            .collect(
                Collectors.toMap(
                    TechnicianIntermediate::getId, TechnicianIntermediate::getStartTime));
    ...
    Map<String, LunchState> lunchStates =
        technicians.stream()
            .collect(Collectors.toMap(TechnicianIntermediate::getId, t -> LunchState.notTaken()));

    for (SampleIntermediate sample : samples) {

      AssignmentIntermediate bestAssignment =
      ...
      long duration =
          Math.round(sample.getAnalysisTime() / bestAssignment.technician().getEfficiency());
      ...
      if (end.isAfter(bestAssignment.technician().getEndTime())) {
        ...
        System.out.println(
            "Shift overflow for: "
                ...
                + end
                + " techEnd="
                + bestAssignment.technician().getEndTime());
        continue;
      }
      ...
      equipmentFreeAt.put(
          bestAssignment.equipment().getId(),
  ...
  private ScheduleEntryIntermediate toEntry(
    ...
    return ScheduleEntryIntermediate.builder()
        ...
        .cleaningDelay(equipment.getCleaningTime())
        .build();
  }
