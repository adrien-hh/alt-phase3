...
public class MetricsCalculatorIntermediate {
  ...
  public MetricsIntermediate computeMetrics(
      LabInputIntermediate input, List<ScheduleEntryIntermediate> schedule, int conflicts) {
  ...
  private long computeTotalTime(List<ScheduleEntryIntermediate> schedule) {
    LocalTime earliestStart =
        schedule.stream().map(ScheduleEntry::getStartTime).min(LocalTime::compareTo).orElse(null);

    LocalTime latestEnd =
        schedule.stream().map(ScheduleEntry::getEndTime).max(LocalTime::compareTo).orElse(null);
            ...
            .mapToLong(
                technician ->
                    schedule.stream()
                        .filter(
                            scheduleEntry ->
                                scheduleEntry.getTechnicianId().equals(technician.getId()))
  ...
  private double computeTechnicianUtilization(
      List<TechnicianIntermediate> technicians, List<ScheduleEntryIntermediate> schedule) {
  ...
  private double computeAverageWaitTime(
      List<SampleIntermediate> samples, List<ScheduleEntryIntermediate> schedule) {
    return roundToOneDecimal(
        schedule.stream()
            .mapToLong(
                entry -> {
                  ...
                  return Duration.between(sample.getArrivalTime(), entry.getStartTime())
                      .toMinutes();
                })
            .average()
            .orElse(0.0));
